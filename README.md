...
## 🌍 公共测试服务器 (Public Test Server)

为了方便大家快速上手和测试程序功能，作者预置了一台公共 Proxy (P端) 服务器：
* **服务器地址**：`www.qzsoft.top`
* **使用说明**：该服务器**允许不鉴权直接使用**，但为了防止资源滥用，**已禁用流量转发（中继）功能**。
* **适用场景**：适合临时使用或体验 UDP P2P 直连打洞效果。如果您的网络环境极其复杂导致 P2P 穿透失败，连接将无法回退到中继模式，此时建议您自行部署专属的 P 端服务器。
